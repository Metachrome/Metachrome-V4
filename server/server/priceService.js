...
class PriceService {
    ...
    initializeWebSocket() {
        try {
            ...
            this.binanceWs = new ws_1.default(wsUrl);
    ...
    async updateMarketDataInDB(priceData) {
        try {
            await storage_1.storage.updateMarketData(priceData.symbol, {
    ...
    async fetchBinancePrices() {
        try {
            ...
            return data
                .filter((ticker) => this.MAJOR_PAIRS.includes(ticker.symbol))
                .map((ticker) => ({
    ...
    simulatePriceMovement(currentPrice, direction, percentage = 0.1) {
