...
async function seedOptionsSettings() {
    ...
    try {
        const existingSettings = await storage_1.storage.getOptionsSettings();
        if (existingSettings.length === 0) {
            for (const setting of optionsSettings) {
                await storage_1.storage.createOptionsSettings(setting);
...
async function seedDemoData() {
    ...
    try {
        ...
        if (adminUser && regularUsers.length > 0) {
            const controlTypes = ['normal', 'win', 'lose'];
            for (let i = 0; i < Math.min(regularUsers.length, 3); i++) {
                const user = regularUsers[i];
                const controlType = controlTypes[i % controlTypes.length];
                const existingControl = await storage_1.storage.getAdminControl(user.id);
                if (!existingControl) {
                    await storage_1.storage.createAdminControl({
                        userId: user.id,
        ...
        if (regularUsers.length > 0) {
            ...
            for (const trade of tradeData) {
                await storage_1.storage.createTrade(trade);
