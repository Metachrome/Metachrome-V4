...
app.post('/api/auth', async (req, res) => {
  ...
  if (walletAddress) {
    ...
    if (!user) {
      ...
      const userData = {
        ...
        password: 'metamask-auth', // Placeholder for MetaMask users
      ...
      try {
        ...
        const newUser = {
          ...
          trading_mode: dbUser.trading_mode || 'normal',
          wallet_address: walletAddress,
        ...
        users.push(newUser);
        user = newUser;
        console.log('✅ New wallet user created and saved to database:', user.username, 'ID:', user.id);
      } catch (error) {
        console.error('❌ Failed to create wallet user in database:', error);
        ...
        users.push(newUser);
        user = newUser;
        console.log('⚠️ Wallet user created in memory only (database failed)');
      }
    } else {
      // Update last login
  ...
  if (email && password && (firstName || lastName || username)) {
    ...
    try {
      ...
      users.push(newUser);
      console.log('✅ New user registered and saved to database:', newUser.username, 'ID:', newUser.id);
    } catch (error) {
      ...
      users.push(newUser);
      console.log('⚠️ User created in memory only (database failed)');
    }
...
app.post('/api/admin/users', async (req, res) => {
  ...
  try {
    ...
    users.push(newUser);
    console.log('✅ User created successfully and saved to database:', newUser.username, 'ID:', newUser.id);
...
app.post('/api/transactions/deposit-request', async (req, res) => {
  ...
  if (token) {
    ...
    user = users.find(u => token.includes(u.id));
  }
  ...
  try {
    ...
    // Generate unique deposit address for this transaction
    const depositId = `deposit-${Date.now()}-${Math.random().toString(36).substring(2, 11)}`;
    ...
    // Store pending deposit (in production, use database)
    if (!global.pendingDeposits) global.pendingDeposits = [];
    global.pendingDeposits.push(pendingDeposit);

    console.log('✅ Deposit request created:', depositId, 'for user:', user.username);
    ...
    res.json({
      ...
      amount: depositAmount,
      currency: currency,
      instructions: getDepositInstructions(currency, depositAddress, depositAmount),
      expiresAt: pendingDeposit.expires_at,
      message: 'Send the exact amount to the provided address'
    });
  } catch (error) {
    console.error('❌ Deposit request failed:', error);
    res.status(500).json({ message: 'Failed to create deposit request: ' + error.message });
  }
});

// Step 2: User submits transaction hash for verification
app.post('/api/transactions/submit-proof', async (req, res) => {
  console.log('🔍 Processing transaction proof:', req.body);

  const { depositId, txHash, walletAddress } = req.body;

  if (!depositId || !txHash) {
    return res.status(400).json({ message: 'Deposit ID and transaction hash are required' });
  }

  try {
    // Find pending deposit
    const pendingDeposit = global.pendingDeposits?.find(d => d.id === depositId);
    if (!pendingDeposit) {
      return res.status(404).json({ message: 'Deposit request not found or expired' });
    }

    // Update pending deposit with proof
    pendingDeposit.tx_hash = txHash;
    pendingDeposit.wallet_address = walletAddress;
    pendingDeposit.status = 'verifying';
    pendingDeposit.submitted_at = new Date().toISOString();

    console.log('✅ Transaction proof submitted:', txHash, 'for deposit:', depositId);

    res.json({
      message: 'Transaction proof submitted successfully. Verification in progress.',
      status: 'verifying',
      estimatedTime: '5-15 minutes'
    });

    // Auto-approve after 30 seconds for demo (in production, verify on blockchain)
    setTimeout(() => {
      approveDeposit(depositId);
    }, 30000);

  } catch (error) {
    console.error('❌ Transaction proof submission failed:', error);
    res.status(500).json({ message: 'Failed to submit proof: ' + error.message });
  }
});

// Step 3: Check deposit status
app.get('/api/transactions/deposit-status/:depositId', async (req, res) => {
  ...
  try {
    ...
    res.json({
      ...
      txHash: pendingDeposit.tx_hash,
      createdAt: pendingDeposit.created_at,
      submittedAt: pendingDeposit.submitted_at,
      completedAt: pendingDeposit.completed_at
    });
  } catch (error) {
...
// Generate deposit address for cryptocurrency
...
// Get deposit instructions for each currency
...
// Auto-approve deposit (in production, this would verify on blockchain)
function approveDeposit(depositId) {
  try {
    const pendingDeposit = global.pendingDeposits?.find(d => d.id === depositId);
    if (!pendingDeposit || pendingDeposit.status !== 'verifying') {
      return;
    }

    // Find user and update balance
    const user = users.find(u => u.id === pendingDeposit.user_id);
    if (!user) {
    ...
    const success = balanceManager.updateBalance(
      ...
      'deposit',
      `Crypto deposit - ${pendingDeposit.amount} ${pendingDeposit.currency}`,
      {
        cryptoDeposit: true,
        currency: pendingDeposit.currency,
        txHash: pendingDeposit.tx_hash,
    ...
    if (success) {
      ...
      pendingDeposit.completed_at = new Date().toISOString();
      ...
      const transaction = {
        ...
        type: 'deposit',
        amount: pendingDeposit.amount,
        symbol: pendingDeposit.currency,
        status: 'completed',
        description: `Crypto deposit - ${pendingDeposit.amount} ${pendingDeposit.currency}`,
      ...
      console.log('✅ Deposit approved and processed:', depositId, 'Amount:', pendingDeposit.amount, pendingDeposit.currency);
      ...
      pendingDeposit.error = 'Failed to update balance';
      console.error('❌ Failed to update balance for deposit:', depositId);
    }
  } catch (error) {
