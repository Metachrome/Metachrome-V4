-- METACHROME LIVE CHAT SYSTEM SETUP
-- Run this SQL in your Supabase dashboard to create chat system tables
...
-- 2. Create chat_messages table (enhanced version)
...
-- 3. Create chat_faq table for chatbot
...
CREATE INDEX IF NOT EXISTS idx_chat_faq_category ON public.chat_faq(category);
CREATE INDEX IF NOT EXISTS idx_chat_faq_active ON public.chat_faq(is_active);
...
-- Chat Conversations: Users can create their own conversations
...
-- Chat Conversations: Admins can view all conversations
...
-- Chat Conversations: Admins can update conversations
...
CREATE POLICY "Users can send messages" ON public.chat_messages
  FOR INSERT WITH CHECK (
    sender_id = current_setting('app.current_user_id', true) AND
    EXISTS (
      SELECT 1 FROM public.chat_conversations 
      WHERE id = conversation_id 
      AND user_id = current_setting('app.current_user_id', true)
...
-- Chat Messages: Admins can view all messages
...
-- Chat Messages: Admins can send messages
...
-- Chat Messages: Admins can update messages (mark as read)
...
-- Chat FAQ: Everyone can read FAQ
DROP POLICY IF EXISTS "Everyone can read FAQ" ON public.chat_faq;
CREATE POLICY "Everyone can read FAQ" ON public.chat_faq
  FOR SELECT USING (is_active = true);

-- Chat FAQ: Only admins can manage FAQ
DROP POLICY IF EXISTS "Admins can manage FAQ" ON public.chat_faq;
CREATE POLICY "Admins can manage FAQ" ON public.chat_faq
  FOR ALL USING (
    EXISTS (
      SELECT 1 FROM public.users 
      WHERE id = current_setting('app.current_user_id', true)::TEXT 
